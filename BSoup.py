...
class Beautiful:
    ...
    def get_city(self, response):
        ...
        cities = []
        try:
            divs = soup.find('div', {'class': 'subregions wrap'}).find('div', {'class': 'group first'}).find(
                'ul')
            li = divs.findAll('li')
            for d in li:
                l = d.find('a')
        ...
        try:
            divs = soup.find('div', {'class': 'subregions wrap'}).findAll('div', {'class': 'group'})[1].find(
                'ul')
            li = divs.findAll('li')
            for d in li:
                l = d.find('a')
    ...
    def get_catalog(self, response):
        """
        parses Regions, Districts and Subdistricts, which were chosen by user
        :param response: response object
        :return: list of Regions, Districts or Subdistricts and links on them in Gismeteo
        """
        soup = BS(response, 'lxml')
        links = []
        regions = []
        try:
            divs = soup.find('div', {'class': 'districts subregions wrap'}).find('div', {'class': 'group first'}).find(
                'ul')
            li = divs.findAll('li')
            for d in li:
                l = d.find('a')
            ...
            raise ValueError('Попробуйте еще раз')
        try:
            divs = soup.find('div', {'class': 'districts subregions wrap'}).findAll('div', {'class': 'group'})[1].find(
                'ul')
            li = divs.findAll('li')
            for d in li:
                l = d.find('a')
    ...
    def get_dates(self, response):
        """
        takes all dates from page
        :param response: response object
        :return: list of dates and lincs
        """
        try:
            soup = BS(response, 'lxml')
            dates = []
            th = soup.findAll('th', {'class': 'date workday'})
            for t in th:
                dates.append(t.text)
            th = soup.findAll('th', {'class': 'date weekend'})
            for t in th:
                dates.append(t.text)
            return dates
        except:
